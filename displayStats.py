...
class DisplayStats():
    ...
    def __init__(self):
        ...
        dataFromCsv=open(os.path.join(os.path.join(os.path.dirname(__file__), "data.csv")),"r")
        allData={}
        for line in dataFromCsv:
            if " = " in line: 
                line = line.strip()  # Remove leading/trailing whitespace
                par, parValue = line.split(" = ", 1)  # Split only once
                par = par.strip()  # Clean key
                parValue = parValue.strip()  # Clean value
                if "." in parValue:
                    allData[par] = float(parValue)
                elif parValue=="None":
                    allData[par]=None
                else:
                    allData[par] = int(parValue)
                # save all game parameters (eg. time, energy, cash...)
                setattr(self, par, allData[par])
                print (f"{par} is {parValue}")
        dataFromCsv.close()

        # self.time = allData["time"]
        # self.energy = 100
        # self.cash = 100000000
        # self.job = 0        
        # self.credit = 0
